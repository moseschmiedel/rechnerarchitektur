...
void sobel(const unsigned char *in_buffer, size_t width, size_t height,
           unsigned char *out_buffer, size_t num_threads) {
#pragma omp parallel for num_threads(num_threads)                              \
    shared(in_buffer, width, height, out_buffer)
...
int main(int argc, char *argv[]) {
    ...
    char* filename = "sobel-parallel.ppm";
    if (argc >= 3) {
        filename = argv[2];
    }
    write_ppm_file(filename, final_image, eisvogel.width,
                   eisvogel.height);
